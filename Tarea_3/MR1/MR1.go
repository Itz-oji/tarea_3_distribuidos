...
func main() {
	...
	producerID := "Falabellox-01"
	regResp, err := client.RegisterProducer(context.Background(), &proto.RegisterRequest{ProducerId: producerID, Store: "Falabellox"})
	if err != nil || regResp == nil || !regResp.Ok {
		log.Fatalf("Registro de productor falló: %v, resp: %v", err, regResp)
	}

	f, err := os.Open("falabellox_catalogo.csv")
	if err != nil {
	...
	if len(items) == 0 {
		log.Fatalf("Catálogo vacío para Falabellox")
	}
	...
	for {
		...
		stock := it.Stock - int32(rand.Intn(5))
		if stock <= 0 {
			stock = 1
		}
