...
func main() {
	...
	producerID := "Riploy-01"
	regResp, err := client.RegisterProducer(context.Background(), &proto.RegisterRequest{ProducerId: producerID, Store: "Riploy"})
	if err != nil || regResp == nil || !regResp.Ok {
		log.Fatalf("Registro de productor falló: %v, resp: %v", err, regResp)
	}

	f, err := os.Open("riploy_catalogo.csv")
	if err != nil {
	...
	for i, row := range records {
		...
		items = append(items, Item{Category: row[2], Product: row[3], Price: precio, Stock: int32(stk64)})
	}
	if len(items) == 0 {
		log.Fatalf("Catálogo vacío para Riploy")
	}
